...
fn diagnostics_shows_app_state_and_capture_source_while_idle() {
    use crate::app::{AudioEndpointKey, AudioEndpointSelection, AudioSourceReading, CaptureState};
...
mod rendering {
    ...
    fn audio_shows_an_honest_localized_empty_state_without_controls() {
        for locale in LOCALES {
            let catalog = Catalog::new(locale);
            let mut harness = page_harness(Page::Audio, locale, TARGET_SIZES[0], populated_state());
            harness.step();
    ...
    mod audio_controls {
        use super::*;
        use crate::app::{
            AudioEndpointChoice, AudioEndpointKey, AudioEndpointRequest, AudioEndpointSelection,
            AudioSourceReading, CaptureState, LatencyChoice, LatencyOption, LatencyReading,
            LatencyUnavailable,
        };
        ...
        fn only_the_reported_capabilities_are_drawn() {
            ...
            harness.step();
            let announced = crate::ui::components::test_support::accessible_strings(harness.root());
            ...
            for locale in LOCALES {
                ...
                let offered: Vec<accesskit::Role> = harness
                    ...
                    .filter(|node| {
                        node.accesskit_node()
                            .label()
                            .is_some_and(|label| label.contains(catalog.text(TextKey::LatencyLow)))
                    })
...
mod live_diagnostics {
    use super::*;
    use crate::app::{
        BufferDiagnosticsReading, DiagnosticIssue, DiagnosticReceiverState, LiveDiagnosticsReading,
    ...
    fn signal_present_waits_for_send_measurements_before_the_healthy_verdict() {
        ...
        harness.step();
        harness
            .get_by_label("Windows input signal is present. Waiting for local send measurements.");
        assert!(harness
...
mod settings_groups {
    ...
    fn announced_with_advanced(page: Page, locale: ResolvedLocale, enabled: bool) -> Vec<String> {
        let mut state = populated_state();
        state.advanced_information = enabled;
...
mod settings_controls {
    ...
    fn the_groups_appear_in_the_supported_specification_order() {
        for locale in LOCALES {
            let catalog = Catalog::new(locale);
            let harness = page_harness(Page::Settings, locale, TARGET_SIZES[0], populated_state());
    ...
    fn the_advanced_switch_is_a_track_at_the_left_edge_of_its_group() {
        for locale in LOCALES {
            ...
            let marks: Vec<egui::Rect> = painted_rects(&harness)
                .into_iter()
                .filter(|(rect, _, _)| control.contains_rect(*rect) && rect.area() < control.area())
                .map(|(rect, _, _)| rect)
                .collect();
            assert!(
                !marks.is_empty(),
