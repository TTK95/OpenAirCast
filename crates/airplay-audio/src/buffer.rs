...
use crate::diagnostics::{AudioDiagnosticsSource, SenderBufferCounters};
...
///
/// Counters live in an [`Arc<SenderBufferCounters>`] shared with any
/// [`AudioDiagnosticsSource`] handed out by
/// [`AudioBuffer::diagnostics_source`], so diagnostics observe real state
/// without locking the audio path.
pub struct AudioBuffer {
    frames: VecDeque<AudioFrame>,
    format: AudioFormat,
    capacity_frames: usize,
    stats: Arc<SenderBufferCounters>,
}
...
impl AudioBuffer {
    ...
    pub fn new(format: AudioFormat, capacity_ms: u32) -> Self {
        ...
        Self {
            ...
            stats: Arc::new(SenderBufferCounters::default()),
        }
    }
    ...
    pub fn push(&mut self, frame: AudioFrame) -> Result<(), BufferError> {
        ...
        self.stats
            ...
            .fetch_add(sample_count, std::sync::atomic::Ordering::Relaxed);
        self.stats
            .queued_samples_per_channel
            .fetch_add(sample_count, std::sync::atomic::Ordering::Relaxed);
        self.frames.push_back(frame);
        self.stats.queued_frames.store(
    ...
    ///
    /// Popping from an empty buffer is the underrun site: it increments the
    /// shared `underrun_events_total` counter exactly once per empty read.
    pub fn pop(&mut self) -> Option<AudioFrame> {
        let frame = match self.frames.pop_front() {
            Some(frame) => frame,
            None => {
                // Read hit an empty ring: this IS the underrun event.
                self.stats
        ...
        self.stats
            .queued_samples_per_channel
            .fetch_sub(sample_count, std::sync::atomic::Ordering::Relaxed);
        self.stats.queued_frames.store(
            self.frames.len() as u32,
            std::sync::atomic::Ordering::Relaxed,
        );
    ...
    ///
    /// Cleared frames leave the queue without counting as reads or writes;
    /// only the queued mirrors are reset.
    ...
    ///
    /// Flushed frames leave the queue without counting as reads (pre-existing
    /// semantics); queued mirrors reset, monotonic totals stay untouched.
    pub fn flush(&mut self) -> Vec<AudioFrame> {
        let flushed = self.frames.drain(..).collect();
        self.stats
            .queued_samples_per_channel
            .store(0, std::sync::atomic::Ordering::Relaxed);
        self.stats
    ...
    /// Create a diagnostics source observing this buffer's live state.
    ///
    /// The returned source reports real values in its `sender_buffer` half;
    /// the `capture_queue` half stays all-zero because a ring buffer has no
    /// capture queue ("not applicable", not fabricated numbers).
    ///
    /// `buffered_ns` is computed from the exact queued-sample mirror via
    /// integer math (`samples * 1e9 / sample_rate_hz`, u128 intermediate),
    /// not from the millisecond-rounded [`AudioBuffer::buffered_ms`].
    pub fn diagnostics_source(&self, sample_rate_hz: u32) -> AudioDiagnosticsSource {
        AudioDiagnosticsSource::sender_buffer(
...
mod tests {
    ...
    mod buffer_operations {
        ...
        fn flush_returns_all_frames() {
            ...
            for i in 0..5 {
                buffer
                    .push(AudioFrame::new(vec![i as i16; 704], i as u64))
                    .unwrap();
            }
