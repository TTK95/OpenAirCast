...
use crate::diagnostics::{RetransmitOutcome, TargetTransportCounters};
...
pub struct RtpSender {
    ...
    /// Per-target transport diagnostics counters (optional; attached by the
    /// streamer at target registration).
    transport_counters: Option<Arc<TargetTransportCounters>>,
}
...
impl RtpSender {
    ...
    /// Attach per-target transport counters for diagnostics.
    ///
    /// Once attached, every data/sync datagram send updates the counters:
    /// `attempted` before the syscall, accepted/bytes/failures afterwards.
    pub fn set_transport_counters(&mut self, counters: Arc<TargetTransportCounters>) {
    ...
    pub fn send(&mut self, packet: &RtpPacket) -> Result<()> {
        ...
        if let Some(ref counters) = self.transport_counters {
            counters.bump_data_attempted();
        }
        let sent = socket.send_to(&packet.serialize(), self.dest);
        if let Some(ref counters) = self.transport_counters {
            counters.record_data_outcome(&sent);
        }
        sent?;
    ...
    pub fn send_audio(
        ...
        let sent = socket.send_to(&serialized, self.dest);
        if let Some(ref counters) = self.transport_counters {
            counters.record_data_outcome(&sent);
        }
        sent?;
    ...
    /// Transmit already prepared audio wire bytes to this target's data port.
    ///
    /// The bytes must come from this sender's own [`Self::prepare_audio`]:
    /// sequence numbering, encryption and packet history were already applied
    /// there.
    pub fn send_prepared_audio(&self, wire: &[u8]) -> Result<()> {
        let socket = self.socket.as_ref().ok_or_else(|| {
            Error::Streaming(airplay_core::error::StreamingError::Encoding(
                "Socket not bound".into(),
            ))
        ...
        let sent = socket.send_to(wire, self.dest);
        if let Some(ref counters) = self.transport_counters {
            counters.record_data_outcome(&sent);
        }
        sent?;
    ...
    /// Transmit an already prepared sync packet to this target's control port.
    ///
    /// Uses the declared control socket when present so the receiver sees the
    /// source port it negotiated, exactly like [`Self::send_sync`].
    pub fn send_prepared_sync(&self, wire: &[u8]) -> Result<()> {
        ...
        if let Some(ref counters) = self.transport_counters {
            counters.bump_sync_attempted();
        }
        let sent = socket.send_to(wire, dest);
        if let Some(ref counters) = self.transport_counters {
            counters.record_sync_outcome(&sent);
        }
        sent?;
        Ok(())
    }
    ...
    pub fn send_sync(&mut self, rtp_timestamp: u32, ntp_timestamp: u64) -> Result<()> {
        ...
        let sent = socket.send_to(&packet, dest);
        if let Some(ref counters) = self.transport_counters {
            counters.record_sync_outcome(&sent);
        }
        sent?;
        tracing::debug!(
            "Sync packet sent: rtp_ts={}, ntp_ts={}",
            rtp_timestamp,
            ntp_timestamp
        );
    ...
    ///
    /// `ntp_timestamp` is **this target's** presentation time for
    /// `rtp_timestamp`: the sync packet is the only place a per-receiver
    /// calibration delay may appear. `rtp_timestamp` itself belongs to the
    /// shared RTP stream and is identical for every group member.
    ///
    /// The sync sequence counter and the first-sync extension bit are part of
    /// this sender's own state, so every target has to build its own packet
    /// even when the presentation times happen to coincide.
    pub fn prepare_sync(
        &mut self,
        rtp_timestamp: u32,
        ntp_timestamp: u64,
    ...
    ///
    /// `ptp_clock_ns` is **this target's** presentation time on the shared
    /// master timeline; `current_rtp_ts`, `next_rtp_ts` and `master_clock_id`
    /// belong to the shared stream and are identical for every group member.
    ...
    pub fn send_ptp_sync(
        ...
        sent?;
        tracing::debug!("PTP sync packet sent: rtp_ts={}", current_rtp_ts);
    ...
    ///
    /// Looks up each requested sequence number in the packet history and sends
    /// retransmit responses (payload type 86) to the control port. Every
    /// requested slot is classified truthfully in the returned outcome; a
    /// send error on one slot never aborts the remaining slots. Without a
    /// bound socket, history-present slots count as send failures.
    pub fn handle_retransmit(&self, request: &RetransmitRequest) -> RetransmitOutcome {
        let socket = self.control_socket.as_ref().or(self.socket.as_ref());
        let dest = self.control_dest.unwrap_or(self.dest);
        let mut outcome = RetransmitOutcome {
        ...
        for i in 0..request.count {
            ...
            // History lookup first: a missing or stale slot is a miss
            // regardless of transport availability.
            let original = match &self.packet_history[idx] {
                Some(original)
                    if original.len() >= 4
                        && u16::from_be_bytes([original[2], original[3]]) == seq =>
                {
                    original
                }
                _ => {
                    outcome.history_misses = outcome.history_misses.saturating_add(1);
                    continue;
                }
            };

            let Some(socket) = socket else {
                outcome.send_failures = outcome.send_failures.saturating_add(1);
                continue;
            };

            let response = build_retransmit_response(original);
            // Retransmit responses are intentionally NOT counted in the
            // data/sync transport counters (they are neither regular
            // audio data nor timing traffic).
            match socket.send_to(&response, dest) {
                Ok(n) if n > 0 => {
                    outcome.accepted_local = outcome.accepted_local.saturating_add(1);
                }
                Err(_) => {
                    outcome.send_failures = outcome.send_failures.saturating_add(1);
                }
        ...
        if outcome.accepted_local > 0 {
            tracing::debug!(
                ...
                outcome.requested_slots
            );
        }
        if outcome.send_failures > 0 {
            tracing::debug!(
                "Retransmit: {} of {} requested slots failed to send",
...
mod tests {
    ...
    /// Test seam mirroring the production accounting sequence
    /// (attempted -> send -> outcome) around an injectable send function.
    /// Used to exercise the failure buckets deterministically, because a
    /// synchronous UDP send failure is not reliably reproducible across
    /// platforms without network flakiness.
    fn counting_send(
        counters: &TargetTransportCounters,
        sync: bool,
        send_impl: &mut dyn FnMut() -> std::io::Result<usize>,
    ) -> std::io::Result<usize> {
        if sync {
            counters.bump_sync_attempted();
        } else {
    ...
    mod transport_counters {
        use super::*;
        use crate::diagnostics::TargetTransportCounters;

        #[test]
        fn counting_send_separates_data_and_sync_buckets() {
            let counters = TargetTransportCounters::new();

            let ok = counting_send(&counters, false, &mut || Ok(20));
            assert_eq!(ok.unwrap(), 20);
            let ok2 = counting_send(&counters, false, &mut || Ok(12));
            assert_eq!(ok2.unwrap(), 12);
            let err = counting_send(&counters, false, &mut || Err(std::io::Error::other("boom")));
            assert!(err.is_err());
            // Ok(0) counts as neither accepted nor failed (truthful).
            let zero = counting_send(&counters, false, &mut || Ok(0));
            assert_eq!(zero.unwrap(), 0);

            let sync_ok = counting_send(&counters, true, &mut || Ok(28));
            assert_eq!(sync_ok.unwrap(), 28);
            let sync_err =
                counting_send(&counters, true, &mut || Err(std::io::Error::other("down")));
            assert!(sync_err.is_err());

            let snap = counters.snapshot();
            assert_eq!(snap.data_datagrams_attempted_total, 4);
            assert_eq!(snap.data_datagrams_accepted_local_total, 2);
            assert_eq!(snap.data_send_failures_total, 1);
            assert_eq!(snap.data_bytes_sent_total, 32);

            assert_eq!(snap.sync_datagrams_attempted_total, 2);
            assert_eq!(snap.sync_datagrams_accepted_local_total, 1);
            assert_eq!(snap.sync_send_failures_total, 1);
            assert_eq!(snap.sync_bytes_sent_total, 28);
        }
    }
