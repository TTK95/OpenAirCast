...
fn test_continuous_streaming_simulation() {
    ...
    // Wait for the pipeline result instead of assuming every platform can run
    // the test scheduler at audio packet rate. The decoder intentionally uses
    // a 5 ms receive timeout, and Windows may round that wait up under load.
    // Five seconds is only a failure bound; the test exits as soon as the
    // observable pipeline condition is satisfied.
    let deadline = Instant::now() + Duration::from_secs(5);
    while total_encoded.load(Ordering::Relaxed) < 200 && Instant::now() < deadline {
    ...
    // The pipeline must sustain enough capture and encoding work to prove
    // continuous forward progress. Exact wall-clock packet pacing belongs to
    // AudioStreamer's scheduler tests, not this cross-platform data-flow test.
