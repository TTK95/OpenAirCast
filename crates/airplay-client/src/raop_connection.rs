...
use crate::connection::{ConnectionWorkers, WorkerLedger};
...
pub struct RaopConnection {
    ...
    render_delay_ms: u32,
    /// Owned background workers (cancellation token; no spawns today).
...
impl RaopConnection {
    ...
    ///
    /// Idempotent: a second call returns `Ok(())` without protocol traffic.
    /// Cancels owned workers (cancel-first) before teardown.
...
impl Drop for RaopConnection {
    fn drop(&mut self) {
        // Drop only signals cancellation; never joins or blocks.
