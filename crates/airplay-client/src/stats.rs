...
impl StatsSnapshot {
    ...
    /// # Compatibility note
    ///
    /// Kept for source compatibility with existing consumers. The new
    /// diagnostics source ([`crate::diagnostics::ClientDiagnosticsSnapshot`])
    /// deliberately has no field or method named "loss" and never calls this:
    /// loss percentages are not part of the truthful diagnostics contract.
