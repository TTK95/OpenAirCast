//! Stream configuration for RTSP SETUP and the sender's local audio buffer.
...
/// Validated capacity for the sender's local audio buffer, in milliseconds.
///
/// This is a local resource setting. It is not an RTSP SETUP parameter and
/// does not make any claim about receiver or hardware availability.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct SenderBufferCapacity(u32);

impl SenderBufferCapacity {
    /// Smallest supported local buffer capacity, in milliseconds.
    pub const MIN_MILLIS: u32 = 100;
    /// Largest supported local buffer capacity, in milliseconds.
    pub const MAX_MILLIS: u32 = 10_000;

    /// Creates a capacity when it is within the local resource bounds.
    ...
    /// Returns the local buffer capacity in milliseconds.
    pub fn millis(self) -> u32 {
        self.0
    }
}

impl Default for SenderBufferCapacity {
    fn default() -> Self {
        Self(2_000)
    }
}

/// Stream configuration for RTSP SETUP plus local sender settings.
///
/// `sender_buffer` configures only the sender's in-process audio buffer; it
/// is deliberately not serialized into RTSP parameters.
#[derive(Debug, Clone)]
pub struct StreamConfig {
    ...
    /// Capacity of the sender's local audio buffer.
    pub sender_buffer: SenderBufferCapacity,
...
impl Default for StreamConfig {
    fn default() -> Self {
        Self {
            ...
            sender_buffer: SenderBufferCapacity::default(),
            asc: None,
        }
    }
}
...
mod tests {
    ...
    mod stream_config {
        ...
        #[test]
        fn sender_buffer_capacity_defaults_to_2000_ms_and_enforces_resource_bounds() {
            assert_eq!(SenderBufferCapacity::default().millis(), 2_000);
            assert_eq!(SenderBufferCapacity::from_millis(99), None);
            assert_eq!(SenderBufferCapacity::from_millis(100).unwrap().millis(), 100);
            assert_eq!(SenderBufferCapacity::from_millis(10_000).unwrap().millis(), 10_000);
            assert_eq!(SenderBufferCapacity::from_millis(10_001), None);
        }
        ...
        fn default_is_realtime_ntp() {
            ...
            assert_eq!(config.sender_buffer.millis(), 2_000);
        }
