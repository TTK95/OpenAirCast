...
/// Read an OPTIONAL TXT field, degrading to `None` when it cannot be read.
///
/// A receiver that advertises a field this parser does not understand is a
/// receiver with one unknown field — never a missing receiver. Only identity
/// fields (`deviceid` for AirPlay, the `MAC@name` form for RAOP) may reject a
/// record, because without identity there is nothing to key the receiver on.
///
/// The discarded value is logged at debug level so an unrecognised format is
/// still discoverable without turning every firmware change into a device
    ...
    match parse(raw) {
        ...
        Err(error) => {
            debug!(
                ...
                value = %raw,
                %error,
                "ignoring TXT field that could not be read"
            );
            None
        }
    }
...
/// Parse one of the group UUID fields (`gid`, `pgid`, `tsid`).
///
/// Paired HomePods advertise a COMPOSITE value of the form `UUID1+UUID2`
/// (tight-sync / stereo pair notation). Neither half is kept: across three
/// paired HomePods on one network both halves differ per device and neither
/// matches any other receiver's `gid`, so there is no evidence that the first
/// UUID is the receiver's own group. Guessing would put a wrong group
/// identity into the session; "unknown" is the honest answer.
fn parse_group_uuid(value: &str) -> Result<uuid::Uuid, ParseError> {
...
impl TxtRecordParser {
    ...
    pub fn parse_airplay_txt(
        ...
        let features = optional_field("features", txt.get("features"), Features::from_txt_value)
            .unwrap_or_default();
        ...
        let source_version =
            optional_field("srcvers", txt.get("srcvers"), Version::parse).unwrap_or_default();

        // Parse Ed25519 public key (optional, 64 hex chars = 32 bytes)
        let public_key = optional_field("pk", txt.get("pk"), Self::parse_public_key);
        ...
        let required_sender_features =
            optional_field("rsf", txt.get("rsf"), Features::from_txt_value);
        ...
        // Parse group UUID (composite "UUID1+UUID2" pairs stay unknown)
        let group_id = optional_field("gid", txt.get("gid"), parse_group_uuid);
        ...
        let parent_group_id = optional_field("pgid", txt.get("pgid"), parse_group_uuid);
        ...
        let tight_sync_id = optional_field("tsid", txt.get("tsid"), parse_group_uuid);
    ...
    pub fn parse_raop_txt(
        ...
        // Parse features: prefer 'ft' (full 64-bit) over 'sf' (legacy 32-bit
        // status flags). An unreadable mask degrades to "no known features"
        // rather than discarding the receiver.
        let features = if txt.contains_key("ft") {
            // Modern RAOP records have 'ft' with full 64-bit features
            optional_field("ft", txt.get("ft"), Features::from_txt_value).unwrap_or_default()
        } else {
            // Legacy fallback to 'sf' field
            optional_field("sf", txt.get("sf"), Self::parse_legacy_features).unwrap_or_default()
        };
        ...
        let source_version = if txt.contains_key("vs") {
            optional_field("vs", txt.get("vs"), Version::parse)
        } else {
            optional_field("vn", txt.get("vn"), Version::parse)
        }
        .unwrap_or_default();
    ...
    pub fn merge_device_info(airplay: &Device, raop: &Device) -> Device {
        ...
        Device {
            ...
            addresses,
            port: airplay.port,
            // Prefer AirPlay features (64-bit), but never let an empty mask
            // from a degraded AirPlay record erase capabilities the RAOP
            // record's 'ft' field states — that would hide PTP support.
            features: if airplay.features.raw() != 0 {
                airplay.features
            } else {
                raop.features
            },
...
mod tests {
    ...
    /// An unreadable OPTIONAL field must never discard the whole receiver.
    ///
    /// Every case here is taken from, or modelled on, a record a real device
    /// put on the wire. The invariant under test is the same for all of them:
    /// `deviceid` (identity) is the only field whose failure may reject the
    /// device; anything else degrades to "not known" and the device survives.
    mod optional_fields_never_reject_device {
        use super::*;

        /// Composite `gid` as advertised by a HomePod that is part of a
        /// tight-sync/stereo pairing: two UUIDs joined by '+'.
        const COMPOSITE_UUID: &str =
            "D38ADA2B-9C22-49E7-8326-8F08E5C0A5FD+55FCD15F-2994-4EBC-8175-B34987203608";

        /// Real `_airplay._tcp` TXT record of the HomePod "Büro", captured
        /// with `cargo run -p airplay-discovery --example debug_devices`.
        fn real_homepod_txt() -> HashMap<String, String> {
            make_txt(&[
                ("acl", "0"),
                ("btaddr", "46:91:1E:C0:79:64"),
                ("c", "2"),
                ("deviceid", "4A:C0:87:A8:7B:29"),
                ...
                ("osvers", "26.6"),
                ("pi", "73870ca5-dc82-4392-a301-dab64165d739"),
                (
                    "pk",
                    "ef449709890441350c7d17fcc5e4f398521ebefe7f76d7dc1ea0722bd8e8ec55",
                ),
                ("protovers", "1.1"),
                ("psi", "49C087A8-7B29-4E96-B490-CD8B899D33D5"),
                ("srcvers", "960.13.1"),
                ("vv", "1"),
            ])
        }
        ...
        #[test]
        fn homepod_with_composite_gid_is_still_a_device() {
            let txt = real_homepod_txt();
            let addr = IpAddr::V4(Ipv4Addr::new(192, 168, 178, 80));

            let device = TxtRecordParser::parse_airplay_txt("Büro", &txt, vec![addr], 7000)
                .expect("a composite gid must not discard the receiver");

            // The composite value is not a UUID, so the group is unknown …
            assert!(device.group_id.is_none());
            // … but every other field of the AirPlay record survives.
            assert_eq!(device.id.0, [0x4A, 0xC0, 0x87, 0xA8, 0x7B, 0x29]);
            assert_eq!(device.model, "AudioAccessory5,1");
            assert_eq!(device.features.raw(), 0x3C35_4BD0_4A7F_CA00);
            assert!(device.features.supports_ptp());
            assert!(device.is_group_leader);
            assert!(device.group_contains_discoverable_leader);
            assert_eq!(
                device.pairing_identity.as_deref(),
                Some("73870ca5-dc82-4392-a301-dab64165d739")
            );
            assert_eq!(
                device.system_pairing_identity.as_deref(),
                Some("49C087A8-7B29-4E96-B490-CD8B899D33D5")
            );
            assert_eq!(
                device.bluetooth_address.as_deref(),
                Some("46:91:1E:C0:79:64")
            );
            assert_eq!(device.access_control, Some(0));
            assert_eq!(device.protocol_version.as_deref(), Some("1.1"));
            assert_eq!(device.status_flags, 0x98404);
            assert!(device.public_key.is_some());
        }

        #[test]
        fn composite_parent_group_and_tight_sync_ids_are_unknown_not_fatal() {
            let mut txt = real_homepod_txt();
            txt.insert("pgid".into(), COMPOSITE_UUID.into());
            txt.insert("tsid".into(), COMPOSITE_UUID.into());

            let device = TxtRecordParser::parse_airplay_txt("Büro", &txt, vec![], 7000)
                .expect("composite pgid/tsid must not discard the receiver");
        ...
        #[test]
        fn unreadable_features_leave_the_device_without_features() {
            let txt = make_txt(&[
                ("deviceid", "AA:BB:CC:DD:EE:FF"),
                ("features", "not-a-mask"),
            ]);

            let device = TxtRecordParser::parse_airplay_txt("Device", &txt, vec![], 7000)
                .expect("unreadable features must not discard the receiver");
            assert_eq!(device.features.raw(), 0);
        }

        #[test]
        fn unreadable_source_version_leaves_the_device_without_version() {
            let txt = make_txt(&[("deviceid", "AA:BB:CC:DD:EE:FF"), ("srcvers", "beta")]);

            let device = TxtRecordParser::parse_airplay_txt("Device", &txt, vec![], 7000)
                .expect("unreadable srcvers must not discard the receiver");
            assert_eq!(device.source_version, Version::default());
        }

        #[test]
        fn unreadable_public_key_leaves_the_device_without_key() {
            let txt = make_txt(&[("deviceid", "AA:BB:CC:DD:EE:FF"), ("pk", "deadbeef")]);

            let device = TxtRecordParser::parse_airplay_txt("Device", &txt, vec![], 7000)
                .expect("unreadable pk must not discard the receiver");
            assert!(device.public_key.is_none());
        }

        #[test]
        fn unreadable_required_sender_features_leave_the_field_unknown() {
            let txt = make_txt(&[("deviceid", "AA:BB:CC:DD:EE:FF"), ("rsf", "zzz")]);

            let device = TxtRecordParser::parse_airplay_txt("Device", &txt, vec![], 7000)
                .expect("unreadable rsf must not discard the receiver");
            assert!(device.required_sender_features.is_none());
        }

        #[test]
        fn missing_deviceid_is_still_the_one_fatal_case() {
            let txt = make_txt(&[("features", "0x200")]);

            let result = TxtRecordParser::parse_airplay_txt("Device", &txt, vec![], 7000);
            assert!(matches!(result, Err(ParseError::MissingField("deviceid"))));
        }

        #[test]
        fn unreadable_raop_optional_fields_never_reject_the_device() {
            let txt = make_txt(&[
                ("ft", "not-a-mask"),
                ("vs", "beta"),
                ("pk", "deadbeef"),
            ...
            let device = TxtRecordParser::parse_raop_txt("AABBCCDDEEFF@Büro", &txt, vec![], 7000)
                .expect("unreadable RAOP optionals must not discard the receiver");

            assert_eq!(device.name, "Büro");
            assert_eq!(device.model, "AudioAccessory5,1");
            assert_eq!(device.features.raw(), 0);
            assert_eq!(device.source_version, Version::default());
            assert!(device.public_key.is_none());
        ...
        #[test]
        fn raop_features_survive_when_the_airplay_record_has_none() {
            // Degraded AirPlay record (features unreadable) merged with a
            // modern RAOP record carrying the full 64-bit 'ft' mask: the
            // capability must come from whichever record actually has it,
            // otherwise the receiver looks incapable of PTP.
            let airplay_txt = make_txt(&[("deviceid", "AA:BB:CC:DD:EE:FF"), ("features", "junk")]);
            let airplay =
                TxtRecordParser::parse_airplay_txt("Büro", &airplay_txt, vec![], 7000).unwrap();

            let raop_txt = make_txt(&[("ft", "0x4A7FCA00,0x3C354BD0")]);
            let raop =
                TxtRecordParser::parse_raop_txt("AABBCCDDEEFF@Büro", &raop_txt, vec![], 7000)
                    .unwrap();

            let merged = TxtRecordParser::merge_device_info(&airplay, &raop);
            assert_eq!(merged.features.raw(), 0x3C35_4BD0_4A7F_CA00);
            assert!(merged.features.supports_ptp());
        }
    }
