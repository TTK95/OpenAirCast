...
/// ```
///
/// - Length: 2 bytes, little-endian, size of the plaintext/ciphertext
    ...
    pub fn encrypt_raw(&mut self, plaintext: &[u8]) -> Result<Vec<u8>> {
        if plaintext.len() > 0x400 {
            return Err(Error::Crypto(CryptoError::Encryption(
                "Raw control messages are limited to one 1024-byte block".to_string(),
            )));
        }
...
mod tests {
    ...
    fn parse_frame_length_works() {
        let data = [0x14, 0x00, 0x01, 0x02, 0x03]; // plaintext length = 20
