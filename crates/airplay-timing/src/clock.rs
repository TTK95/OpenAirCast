...
pub struct ClockOffset {
    /// Offset in nanoseconds, measured as `local - master`.
    ///
    /// Positive means the **local** clock is ahead of the master. Every
    /// producer in this crate uses this convention — see
    /// `PtpClient::calculate_offset` and the BMCA yield flow, which both
    /// compute `((t2 - t1) + (t3 - t4)) / 2` with `t1`/`t4` from the master
    /// and `t2`/`t3` from the local clock. Convert with
    /// [`crate::diagnostics::local_to_master_ns`], which subtracts.
    pub offset_ns: i64,
    /// Estimated error in nanoseconds.
    pub error_ns: u64,
    /// Legacy-compat round-trip time in nanoseconds (plain `t4 - t1`; not the
    /// one-way estimate — see `diagnostics::TimingMeasurement::mean_path_delay_ns`).
    pub rtt_ns: u64,
}
...
impl Clock {
    ...
    ///
    /// Ambiguous by construction: it adds `offset_ns` without stating which
    /// direction the offset was measured in, and it wraps on under/overflow.
    /// Every producer of a [`ClockOffset`] in this crate uses the explicit
    /// `offset = local - master` convention, so converting a local timestamp
    /// to master time must **subtract**.
    #[deprecated(
        since = "0.1.0",
        note = "sign convention is implicit; use `diagnostics::local_to_master_ns` instead"
    )]
    ...
    mod clock {
        ...
        fn wall_rtp_roundtrip() {
            ...
            for offset_ms in [0, 100, 500, 1000, 5000] {
                ...
                assert!(
                    diff < 1_000_000,
                    "Roundtrip error {} ns for offset {}ms",
                    diff,
                    offset_ms
                );
            }
