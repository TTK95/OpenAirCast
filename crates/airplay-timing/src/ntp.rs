...
use crate::diagnostics::{ptp_measurement, TimingDiagnosticsRecorder};
use crate::{ntp_to_unix, Clock, ClockOffset, TimingProtocol};
    ...
    pub fn parse(data: &[u8]) -> Result<Self> {
        if data.len() < 32 {
            return Err(Error::Parse(
                airplay_core::error::ParseError::InvalidFormat("NTP request too short".into()),
            ));
        }
...
impl NtpResponse {
    ...
    pub fn parse(data: &[u8]) -> Result<Self> {
        if data.len() < 32 {
            return Err(Error::Parse(
                airplay_core::error::ParseError::InvalidFormat("NTP response too short".into()),
            ));
        }
...
impl TimingProtocol for NtpTimingClient {
    ...
    async fn sync(&mut self) -> Result<ClockOffset> {
        ...
        let request = NtpRequest::new(&self.clock).with_sequence(self.sequence);
        ...
        // Diagnostics hook: all four wire timestamps of this exchange are
        // truthfully retained above (t1 = echoed reference time, t2/t3 from
        // the response, t4 = local receive), so they are mirrored through the
        // same math as `ptp_measurement` after NTP-era -> unix-ns conversion.
        // The recorder owns sequence stamping; a failed conversion only
        // skips the record.
        if let (Some(recorder), Some(started)) =
            (self.diagnostics.as_ref(), self.diagnostics_started)
        {
            let elapsed_ns = u64::try_from(started.elapsed().as_nanos()).unwrap_or(u64::MAX);
            if let Some(measurement) = ptp_measurement(
                0,
                elapsed_ns,
                ntp_to_unix(request.reference_time),
                ntp_to_unix(response.receive_time),
                ntp_to_unix(response.send_time),
                ntp_to_unix(recv_time),
            ) {
